...
serve(async (req) => {
  ...
  try {
    ...
    const schema = `-- ============================================
-- VIBE CLASS - Complete Unified Export
...
-- 7. DATA
-- ============================================
${toInsert("modules", modules.data || [])}
${toInsert("videos", videos.data || [])}
${toInsert("video_quizzes", quizzes.data || [])}
${toInsert("missions", missions.data || [])}
${toInsert("squads", squads.data || [])}
${toInsert("squad_members", squadMembers.data || [])}
${toInsert("student_profiles", studentProfiles.data || [])}
${toInsert("student_lesson_progress", progress.data || [])}
${toInsert("student_quiz_attempts", quizAttempts.data || [])}
${toInsert("student_quiz_results", quizResults.data || [])}
${toInsert("student_observations", observations.data || [])}
${toInsert("student_achievements", achievements.data || [])}
${toInsert("student_mission_submissions", submissions.data || [])}
${toInsert("certificates", certificates.data || [])}
${toInsert("squad_mission_submissions", squadSubmissions.data || [])}

-- ============================================
-- DONE! Configure .env with your Supabase URL and keys.
-- ============================================
`;
    ...
    return new Response(fullSQL, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="vibe-class-complete-${new Date().toISOString().slice(0,10)}.sql"`,
      },
    });
  } catch (err) {
